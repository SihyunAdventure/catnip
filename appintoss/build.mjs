// 캣데월드 빌드 — 루트의 catnip 정적 사이트를 dist/ 로 조립하며 토스용 변환을 적용한다.
//
// 변환 내용:
//   1. 브랜딩: <title>·og 의 catnip → 캣데월드, <html lang="ko">
...
//   4. window.__CATDEWORLD__ 플래그 주입 (토스 환경 분기용)
//
...
function transformHtml(src, relPath) {
  ...
  s = s.replace(/<title>([^<]*)<\/title>/, (_, t) => `<title>${t.replaceAll('catnip', BRAND)}</title>`);
  s = s.replaceAll('content="catnip"', `content="${BRAND}"`);
  s = s.replace(/(<a class="wordmark"[^>]*>)catnip(<\/a>)/g, `$1${BRAND}$2`);

  // 미니앱에는 SEO 크롤러 레이어 불필요 — catnip 브랜드 텍스트·GitHub 외부 링크 노출 제거
  s = s.replace(/<section class="about">[\s\S]*?<\/section>\n?/, '');

  // 4. 토스 환경 플래그 (charset 직후 — 모든 스크립트보다 먼저)
  s = s.replace('<meta charset="utf-8">', '<meta charset="utf-8">\n<script>window.__CATDEWORLD__=true;</script>');
