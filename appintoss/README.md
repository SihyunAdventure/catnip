...
| 층위 | 이름 | 왜 |
|---|---|---|
| **appName** (딥링크 ID `intoss://nabi`) | `nabi` | 🔒 콘솔 등록 후 **영구 불변** → 상표 안전값으로 박음. 유저에게 거의 안 보임 |
| **표시명** (콘솔 앱 이름·앱 내 워드마크) | **캣데월드** | 유저가 보는 이름. 교체 가능 → 문제 시 여기만 바꿈 |
| package name · 커밋 · 문서 | **캣데월드** | 우리가 이 프로젝트를 부르는 이름 |
| 빌드 산출물 | `nabi.ait` | `ait build` 가 appName 으로 자동 생성 (제어 불가, 무해) |
| 글로벌 웹 | **Nabi** | 별도 브랜드 (nabi.fun) |
